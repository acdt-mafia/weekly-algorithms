...
int head[MAX];       // 각 노드에서 시작하는 첫 간선의 인덱스. u번 노드의 첫째 자식과 연결되는 간선 번호
int to[MAX * 2];     // 각 간선이 도착하는 노드 번호 //*2인 이유는 무방향이라 저장 2번 해야 해서 배열 늘린 것.
int weight[MAX * 2]; // 각 간선의 가중치
int next[MAX * 2];   // 한 노드에서 이어진 간선들 저장. 같은 부모를 가진 다음 간선의 인덱스. next[2:현재 노드와 이어져있는 이번에 확인하고 있는 간선 번호] = 1: 현재 노드와 이어져 있는 다음에 볼 간선 번호; -1 되면 끝난다.
...
void dfs(int now, int dist)
{
    ...
    for (int i = head[now]; i != -1; i = next[i])
    {
        ...
        // head[now] = 현재 노드 now에서 시작하는 첫 번째 간선 번호
